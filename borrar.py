            ...
            if class_id == 2:  # Taco
                ...
                if x2 > x1 and y2 > y1:
                    ...
                    if contornos:
                        ...
                        cv2.drawContours(frame, [box_global], 0, (0, 0, 255), 2)
...
#------------------------------------------------------------------------------------------------------------------------------------------------
ESTE CPODIGO ES SIN NADA DE PYMUNK PERO CON LA TRAYECTORIA DEL TACO AL 100%
import argparse
import torch
import time
import cv2
...
class VideoProcessor:
    ...
    ## EL QUE MÁS ME SIRVIO (que esté más cerca del centro de la mesa)
    """def handle_detections(self, frame, detections):
        centro_mesa_x, centro_mesa_y = self.calcular_centro_mesa_genérico(frame)
    ...
    # QUE ESTE MÁS CERCA DEL CENTRO DE UNA BOLA   
    def handle_detections(self, frame, detections):
        centro_mesa_x, centro_mesa_y = self.calcular_centro_mesa_genérico(frame)
        centros_bolas = []
