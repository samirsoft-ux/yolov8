...
class VideoProcessor:
    ...
    def distancia_punto_segmento(self, punto, segmento_inicio, segmento_final):
        ...
        t = np.clip(t, 0, 1)  # Limitar t entre 0 y 1 para asegurar que el punto esté dentro del segmento
        punto_cercano = a + t * ab
        distancia = np.linalg.norm(punto_cercano - p)
        return distancia, punto_cercano
    ...
    def calcular_nuevas_velocidades(self, centro1, velocidad1, centro2, velocidad2, masa):
        ...
        # Calcular cambio de velocidad debido a la colisión, asumiendo igual masa y colisión perfectamente elástica
        delta_v = np.dot(velocidad2 - velocidad1, direccion_colision_unitaria) * direccion_colision_unitaria
        velocidad1_nueva = velocidad1 + delta_v
        velocidad2_nueva = velocidad2 - delta_v
    ...
    def simulate_impact_trajectory(self, frame, centro_bola_id, direccion_taco, velocidad_impacto, mesa_corners, current_active_balls, centros_bolas):    
        ...
        while np.linalg.norm(velocidad_actual) > 1:
            ...
            # Verificar colisión con otras bolas
            if not colision_detectada:
                punto_colision_cercana, bola_cercana_id = self.calcular_punto_colision_cercana(punto_actual, punto_siguiente, current_active_balls, centros_bolas, centro_bola_id)

                if punto_colision_cercana is not None:
                    cv2.line(frame, tuple(punto_actual.astype(int)), tuple(np.array(punto_colision_cercana).astype(int)), (0, 255, 0), 2)
                    
                    # Simular transferencia de energía y cálculo de nuevas velocidades
                    centro_bola2 = np.array(centros_bolas[current_active_balls.index(bola_cercana_id)])
                    velocidad_bola2 = np.zeros_like(velocidad_actual)  # Asumir bola estática para simplificar
                    
                    velocidad1_nueva, velocidad2_nueva = self.calcular_nuevas_velocidades(punto_actual, velocidad_actual, centro_bola2, velocidad_bola2, masa)
                    punto_actual = np.array(punto_colision_cercana) + velocidad1_nueva  # Actualizar posición basada en la nueva velocidad
                    
                    velocidad_actual = velocidad1_nueva * 0.9  # Simular la pérdida de velocidad tras la colisión
                    colision_detectada = True

            # Si no hay colisiones, dibujar la trayectoria completa
            if not colision_detectada:
                cv2.line(frame, tuple(punto_actual.astype(int)), tuple((punto_actual + velocidad_actual).astype(int)), (0, 0, 255), 2)
                punto_actual += velocidad_actual

            velocidad_actual *= 0.9  # Reducir velocidad por fricción
    
    """def calcular_punto_colision_cercana(self, punto_inicio, punto_final, current_active_balls, centros_bolas, centro_bola_id):
        distancia_minima = np.inf
        punto_colision_cercano = None
        bola_cercana_id = None

        for i, centro_bola in enumerate(centros_bolas):
            if current_active_balls[i] != centro_bola_id:  # Evitar comparar con la misma bola
                distancia_linea_bola = self.distancia_punto_segmento(np.array(centro_bola), punto_inicio, punto_final)

                if distancia_linea_bola <= self.average_ball_radius * 2 and distancia_linea_bola < distancia_minima:
                    distancia_minima = distancia_linea_bola
                    punto_colision_cercano = centro_bola  # Simplificación, considera ajustar para el punto exacto de colisión
                    bola_cercana_id = current_active_balls[i]

        return punto_colision_cercano, bola_cercana_id"""
    def calcular_punto_colision_cercana(self, punto_inicio, punto_final, current_active_balls, centros_bolas, centro_bola_id):
        distancia_minima = np.inf
        punto_colision_cercano = None
        bola_cercana_id = None
        ...
        for i, centro_bola in enumerate(centros_bolas):
            if current_active_balls[i] != centro_bola_id:  # Evitar comparar con la misma bola
                distancia_linea_bola, punto_cercano = self.distancia_punto_segmento(centro_bola, punto_inicio, punto_final)
                if distancia_linea_bola <= self.average_ball_radius * 2:
                    direccion_colision = np.array(centro_bola) - punto_cercano
                    punto_exacto_colision = punto_cercano + direccion_colision / np.linalg.norm(direccion_colision) * self.average_ball_radius
                    distancia_actual = np.linalg.norm(punto_inicio - punto_exacto_colision)
                    if distancia_actual < distancia_minima:
                        distancia_minima = distancia_actual
                        punto_colision_cercano = punto_exacto_colision
                        bola_cercana_id = current_active_balls[i]
